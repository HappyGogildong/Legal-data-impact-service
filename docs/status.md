...
| Analysis Engine — **Layer A**(SUMMARY·DIFF): `ContextBuilder`(정본→source_id 블록)·`Reasoner` 포트+`SpringAiReasoner`(Opus)·`AnalysisEngine`(조립→추론→인용검증→재생성≤N→폴백) | ✅ 단위 7(ContextBuilder 3·Engine 4, FakeReasoner)·라이브 스모크 옵트인. Layer B·`QueryDispatcher`·차원핸들러·캐시는 후속 |
| 웹 프론트엔드 · User Profile Store | ⬜ |

단위 테스트 **104개**(+AnalysisEngine 7) + 통합 **8건**(실 Postgres/pgvector, Testcontainers: Law Store 3 + 적재 조립 2 + ChunkStore 3) 통과. (실 임베딩·번역·해석 스모크/평가 5종은 옵트인·수동)
