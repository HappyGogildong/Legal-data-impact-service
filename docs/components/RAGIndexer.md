...
> 코퍼스를 임베딩해 **Vector Index에 적재**. 두 네임스페이스: *분석용(현행법·선례)* + *탐색용(법안 요약·LawFacts)*. 관련: [[component-specs]] §4 #4 · [[v0.5-bill-discovery]] §3.4
...
| 입력(탐색용) | 법안 **요약·LawFacts·affectedDomains** | [[Normalizer]]·[[AnalysisEngine]] 산출 |
...
## 동작
1. 코퍼스 → 청크(현행법=조문 단위, 법안=요약/LawFacts)
...
class RAGIndexer:
    ...
    def index_bill(self, bill: Bill, facts: LawFacts) -> None: ... # namespace="bill"
```
...
- **법안 전문은 임베딩하지 않음.** 법안 한 건은 분석 시 컨텍스트에 통째로 들어가므로, 탐색용은 *요약/LawFacts*만(저장·노이즈 절감).
...
- 입력: [[SourceConnector]](현행법), [[Normalizer]]/[[AnalysisEngine]](법안 요약·LawFacts)
