...
> **D42 이관 (2026-08-02):** 해소 대상이 *의안* → **시행예정 법령**으로 바뀌었다. **의안번호 분기는 제거**됐다 — 법령에 의안번호가 없고 사용자가 법령ID(`001809`)를 입력하지도 않는다. 현재 3단계다: **법령명 정확·퍼지 매칭 → 의미검색 → fail-closed 판정**.
> 커넥터 의존도 끊었다. `LawLookup` 포트를 통해 조회하며, 저장소 적재 후 구현만 교체한다(v0.8 §3.2 — 해소는 *적재분*을 읽는다).
> **새 케이스(D43):** 같은 `법령ID`에 시행예정본이 여럿이면 `AMBIGUOUS`로 떨어뜨리고 **어느 시행일 기준인지 되묻는다**. 실측에서 자본시장법이 3건이었다.
