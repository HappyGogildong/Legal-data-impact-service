...
| D45 | **공개 API 표면 고정** — 웹앱↔서버 `/api/v1/*` 6개 엔드포인트(검색·해소 / 시행예정본 목록 / 법령사실+diff / 개인화분석 / 프로필 CRUD). 시행일을 경로에 포함(`/laws/{lawId}/{efYd}`), 해소 4상태는 HTTP 200, Layer B만 인증 필요 | 확정 | `/api/v1/...` 가 6개 문서에서 언급만 되고 어디에도 정의되지 않았다. 내부 계약(`/internal/v1`)과 분리해 *바깥 경계*를 명세. 시행일을 경로에 넣는 것은 D43(같은 lawId 시행예정본 복수) 때문 | [[service-api-spec]] |
