...
| D41 | **페르소나 = 회원가입 자기신고 프로필**(Nemotron 군집 폐기) — `UserProfile{purposes, ageBand, occupation, employmentType, householdType, housingType, regionSido, interests}`. **성명·생년월일·연락처·상세주소 미수집**, 시도까지만. 캐시 키는 `userId`가 아니라 **프로필 속성 해시**. D10 주입 규율(비인용·`<persona>` 격리)은 승계 | 확정 | 고정 6세그먼트는 해상도 부족(같은 버킷 내 상황 상이). 자기신고가 더 정확·최신이고 외부 데이터셋·군집 파이프라인 의존 제거. **주의: 직접식별정보 미수집일 뿐 "개인정보 아님"은 아님** — 조합 재식별·계정 식별자 존재하므로 처리방침·동의·파기 필요 | [[component-specs]] §2 |
