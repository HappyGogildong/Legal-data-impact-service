...
**문제.** 인기 법령에 **완전 동일 질의**(같은 답변 캐시 키 §3.4 — 질문·법령·프로필 모두 동일)가 **미캐시** 상태로 N명 동시에 오면, 같은 Opus 생성이 **N번** 돈다. 비용은 정확히 N배 — ADR-001의 "런타임 LLM 호출이 비용 동인"과 정면으로 충돌한다.
...
**적용 범위.** **완전 동일 질의**의 콜드-중복만(§3.4 답변 캐시). Layer B 생성 대상. **서로 다른 질의는 병합하지 않는다**(D51). context 재사용은 prompt caching이 담당하므로 스탬피드가 아니고, Layer A(SUMMARY·DIFF)는 선계산이라 조회뿐.
