...
문서 버전 0.2 · 2026-05-31 · 상태: **현행(latest)**

> 변경 이력과 버전별 설계 이유는 [`../ARCHITECTURE.md`](../ARCHITECTURE.md) 색인 참고.
> 이 파일은 v0.2 시점의 **동결 스냅샷**이다. 수정하지 말 것 — 새 버전은 새 파일로.
...
- **TypeScript — 웹 프론트엔드 + (선택) MCP 어댑터.** 일반 시민이 실제로 쓰는 경로는 **웹앱**이며 그 프론트엔드를 담당한다. MCP는 참고 리포처럼 Claude·Cursor 등 외부 LLM 클라이언트·파워유저·B2B 연동을 위한 **부가 표면**일 뿐, v1 주 경로가 아니다.

> **스택 결정 (2026-05-31).** 타겟이 일반 시민 **웹앱**이므로 사용자는 MCP를 직접 호출하지 않는다. 무거운 NLP·LLM·RAG는 생태계가 풍부한 **Python**에 두고, 견고한 도메인 로직·트랜잭션·API는 **Spring**에 둔다. 이 분리 덕분에 "LLM 서비스에 Spring이 부적합"하다는 통념은 적용되지 않는다 — Spring에 LLM 생태계를 요구하지 않기 때문이다. 향후 운영 복잡도를 줄이고 싶으면 Spring AI 1.x로 추론을 JVM에 흡수하는 길도 열려 있으나, 현 단계에서는 Python 분리를 유지한다.
...
flowchart TB
    ...
    subgraph EDGE["④ 사용자 표면"]
        ...
        T1["MCP 어댑터 (TypeScript)<br/>(선택) 외부 LLM 클라이언트·B2B"]
    end

    USER(["일반 시민"])
    POWER(["파워유저 / LLM 클라이언트"])
...
데이터는 항상 한 방향으로 흐른다. **출처 → 정규화 → 저장 → 커맨드 후처리 → 사용자 표면**. 일반 시민의 주 경로는 **웹앱 → Spring REST API**이며, MCP는 동일한 REST/커맨드 위에 얹는 선택적 어댑터다. 사용자가 던진 링크·영상도 Source Analyzer가 "이 콘텐츠가 가리키는 법안이 무엇인가"를 식별한 뒤 동일한 커넥터 경로로 합류시킨다. 즉 입력 소스가 늘어나도 하류 구조는 건드리지 않는다.
...
## 5. 사용자 표면 (웹앱 주 경로 + MCP 부가 표면)

일반 시민의 주 경로는 **웹앱**이다. TypeScript 프론트엔드가 Spring 코어의 REST API(`/api/v1/...`)를 호출해 검색·영향분석·행동계획을 화면에 보여준다. 사용자는 MCP의 존재를 알 필요가 없다.

**MCP는 동일한 REST/커맨드 위에 얹는 선택적 어댑터**다. 참고 리포처럼 Claude·Cursor 등 외부 LLM 클라이언트나 B2B 연동에서 같은 기능을 도구로 부르고 싶을 때만 활성화한다. 각 MCP 도구는 코어 커맨드 한 개(또는 체인)에 1:1로 매핑되므로, 새 커맨드를 추가하면 새 MCP 도구를 거의 자동으로 얻는다.

| 표면 | 대상 | 호출 경로 |
|---|---|---|
| 웹앱 프론트엔드 (TS) | 일반 시민 (주 경로) | → Spring REST `/api/v1/analyze` |
| MCP 어댑터 (TS, 선택) | 외부 LLM 클라이언트·파워유저·B2B | → 동일 REST/커맨드 |

| MCP 도구(선택 활성화) | 매핑 커맨드 |
|---|---|
...
신규 **데이터 출처**는 커넥터 추가, 신규 **use-case**는 커맨드 추가, 신규 **클라이언트**는 표면(웹 화면 / MCP 도구) 추가로 각각 독립적으로 늘어난다. 세 축이 서로 결합돼 있지 않다는 점이 이 설계의 요지다.
...
런타임은 Python(추론)·Java Spring(도메인·API)·TypeScript(웹/MCP) 셋이다. 이는 배포 파이프라인 3개와 서비스 간 네트워크 홉을 의미하므로, Python↔Spring 간 계약(REST 스키마)과 관측성(분산 추적)을 초기에 못박아 두는 것이 좋다.
...
5. (선택) MCP 어댑터는 웹앱 MVP 안정화 후 부가 표면으로 추가.
