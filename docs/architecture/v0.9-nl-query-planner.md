...
> 컴포넌트 상세는 [`../components/component-specs.md`](../components/component-specs.md)·[`../components/query/QueryPlanner.md`](../components/query/QueryPlanner.md), 결정 이력은 [`../adr/decision-log.md`](../adr/decision-log.md).
...
`LawConnector`(eflaw/law) → `Normalizer`(조문 병합·부칙 필터·`revision`) → `Diff Builder`(변경 조문 ↔ 시행중본 대조 → `diffVsCurrent`). 상세: [`../components/ingest/SourceConnector.md`](../components/ingest/SourceConnector.md)·[`../components/ingest/Normalizer.md`](../components/ingest/Normalizer.md).
...
번역(`QueryTranslator`, Haiku) → 해소/발견 결정(`QueryPlanner`) → 라우팅(`QueryDispatcher`). 상세·시나리오: [`../components/query/QueryPlanner.md`](../components/query/QueryPlanner.md).
