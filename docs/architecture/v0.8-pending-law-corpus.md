...
> *예외 1건 (2026-08-02): `BillFacts` → `LawFacts` **용어 개명**을 반영했다(D44). 설계 변경이 아닌 순수 개명이고, 현행 스냅샷만 옛 용어를 쓰면 혼선이 남기 때문이다. v0.4~v0.7은 동결을 유지하므로 옛 이름 그대로다.*
...
**흐름:** 스케줄러가 `LawConnector`로 **시행예정 목록**을 가져온다(`efYd` 범위 필터, 실측 899건). 각 건의 본문을 받아 Normalizer가 조문·부칙을 표준화해 Law Store에 넣고, **같은 `법령ID`의 시행중본**을 끌어와 `조문변경여부='Y'` 조문만 대조해 `diffVsCurrent`를 채운다. 그 위에서 LawFacts(Layer A 사실)를 파생하고, 시행중 조문과 LawFacts 요약을 각각 임베딩해 Vector Index에 적재한다.
...
**흐름:** 사용자 입력을 SourceAnalyzer가 해소(4상태)하고 **`RESOLVED`만 분석으로 넘긴다.** Pipeline이 Law·변경조문·LawFacts(오프라인 산출)와 **자기신고 프로필**(D41)을 조립해 커맨드를 실행하고, Engine이 현행법을 RAG 검색해 Opus로 추론한다. **인용검증 게이트**를 통과한 결과만 사용자에게 나가고, 근거 부족이면 폴백한다.
...
> 상세·재현: [`../components/connector/SourceConnector.md`](../components/connector/SourceConnector.md) §MVP 본문 경로 · `tools/probe_eflaw.py`

열린국회(의원발의)·법제처 입법예고는 **커넥터 인터페이스는 유지하되 분석 대상에서 제외**한다(post-MVP). 확장 = 구현체 추가라는 개방-폐쇄 구조는 그대로다.

### 4.2 도메인 모델 (D42로 개정)

**`Law`**(MVP 분석 대상) / `Article` / `LawFacts`(Layer A 캐시) / `ImpactResult`(Layer B). **`Bill`(의안)은 post-MVP로 보류** — `eflaw` 응답에 `billNo`·발의자·소관위·심사단계가 없어 `Bill`을 쓰면 절반이 항상 null이다. 타입은 [`../components/component-specs.md`](../components/component-specs.md) §1.1.
...
| **탐색용** | `pending` | 시행예정 법령 요약·LawFacts | Source Analyzer — 모호 질의 → 후보 |

**적재·검색은 반드시 동일 임베딩 모델**(공유 `Embedder`). 법령 *전문*은 임베딩하지 않는다(분석 시 컨텍스트에 통째로 들어감).

> 개명 2건(대상이 의안이 아니라 시행예정 법령이므로): 탐색용 네임스페이스 `bill` → **`pending`**, 파생 캐시 타입 `BillFacts` → **`LawFacts`**(D44).
...
**2계층:** Layer A(법령 사실·LawFacts·조문 diff — **오프라인 선계산**) → Layer B(`PersonaImpact`·`ActionPlan` — 온라인, 프로필별). Triage 분류 기준은 고정([`../reference/triage-policy.md`](../reference/triage-policy.md)), 차등 라우팅 스테이지는 post-MVP.
...
- **비용 동인:** 저장 용량이 아니라 *띄워 둔 인스턴스 + 런타임 LLM 호출*(ADR-001). 오프라인 선계산(diff·LawFacts·임베딩)과 결과 캐시가 온라인 LLM 호출을 줄이는 핵심 레버. **`조문변경여부` 필터가 여기에 직접 기여**한다.
