...
문서 버전 0.3 · 2026-06-22 · 상태: **현행(latest)**

> 변경 이력과 버전별 설계 이유는 [`../ARCHITECTURE.md`](../ARCHITECTURE.md) 색인 참고.
> 이 파일은 v0.3 시점의 **동결 스냅샷**이다. 수정하지 말 것 — 새 버전은 새 파일로.
...
첫째, **신뢰 가능한 출처에서만 원천 데이터를 가져온다.** 모든 분석 결과는 의안 원문·조문·시행일이라는 일차 출처로 역추적 가능해야 한다(참고 리포의 `verify_citations` 사상). 둘째, **수집과 해석을 분리한다.** "법안 데이터를 정규화해 저장"하는 파이프라인과, "use-case별로 후처리"하는 커맨드 계층은 서로의 내부를 몰라야 한다. 셋째, **확장은 새 커맨드를 추가하는 일이지 코어를 고치는 일이 아니다**(개방-폐쇄 원칙). 넷째, **입력 소스는 다양하다.** API뿐 아니라 사용자가 던지는 뉴스 링크·기사 텍스트도 "법안 식별"이라는 동일한 정규 흐름으로 수렴시킨다.
...
- **Java Spring — 코어 도메인 & 커맨드 오케스트레이션 (주 사용자 경로의 백엔드).** 도메인 모델의 단일 진실 공급원, use-case별 커맨드 레지스트리/파이프라인, 트랜잭션·권한, 웹앱에 직접 노출되는 REST API. 장기 유지보수와 팀 협업에 강함.
- **TypeScript — 웹 프론트엔드 (주 사용자 경로).** 일반 시민이 실제로 쓰는 유일한 사용자 경로는 **웹앱**이며 그 프론트엔드를 담당한다. MCP 어댑터는 내부 구현·테스트·향후 B2B 연동을 위해 **둘 수 있는 선택지**이지만, 현 단계에서 **사용자 인터페이스로는 제공하지 않는다**(노출 계획 미정).

> **스택 결정 (2026-06-22).** 타겟이 일반 시민 **웹앱**이므로 사용자는 MCP를 직접 호출하지 않으며, MCP를 사용자 표면으로 노출할 계획도 아직 없다. 무거운 NLP·LLM·RAG는 생태계가 풍부한 **Python**에 두고, 견고한 도메인 로직·트랜잭션·API는 **Spring**에 둔다. 이 분리 덕분에 "LLM 서비스에 Spring이 부적합"하다는 통념은 적용되지 않는다 — Spring에 LLM 생태계를 요구하지 않기 때문이다. 향후 운영 복잡도를 줄이고 싶으면 Spring AI 1.x로 추론을 JVM에 흡수하는 길도 열려 있으나, 현 단계에서는 Python 분리를 유지한다.
...
flowchart TB
    ...
    subgraph PIPE["① 수집·해석 파이프라인 (Python)"]
        ...
        C2["Source Analyzer<br/>링크/기사 → 법안 식별"]
    ...
    subgraph CORE["③ 코어 도메인 & 커맨드 (Java Spring)"]
        ...
        K4["REST API<br/>(주 사용자 경로)"]
    end

    subgraph EDGE["④ 사용자 표면"]
        W1["웹앱 프론트엔드 (TypeScript)<br/>★ 일반 시민 주 경로"]
    end

    subgraph INTERNAL["(내부) 비노출 어댑터"]
        T1["MCP 어댑터 (TypeScript)<br/>내부 구현·향후 B2B용<br/>사용자 미노출"]
    end
...
데이터는 항상 한 방향으로 흐른다. **출처 → 정규화 → 저장 → 커맨드 후처리 → 사용자 표면**. 일반 시민의 주 경로는 **웹앱 → Spring REST API**이며, MCP 어댑터는 동일한 REST/커맨드 위에 얹을 수 있는 **내부 전용 어댑터**일 뿐 사용자에게 노출되지 않는다. 사용자가 던진 링크·기사도 Source Analyzer가 "이 콘텐츠가 가리키는 법안이 무엇인가"를 식별한 뒤 동일한 커넥터 경로로 합류시킨다. 즉 입력 소스가 늘어나도 하류 구조는 건드리지 않는다.
...
| `SourceResolveCommand` | 링크/기사 → 법안 식별 | 3.3 흐름의 진입점 |

### 4.3 확장 방식

새 use-case = 새 `AnalysisCommand` 구현체 한 개 + 레지스트리 등록(Spring이라면 `@Component`로 자동 발견). 코어·파이프라인·표면은 손대지 않는다.
...
일반 시민의 사용자 경로는 **웹앱 하나**다. TypeScript 프론트엔드가 Spring 코어의 REST API(`/api/v1/...`)를 호출해 검색·영향분석·행동계획을 화면에 보여준다. 사용자는 MCP의 존재를 알 필요가 없다.

**MCP 어댑터는 내부 구현 선택지로만 둔다.** 동일한 REST/커맨드 위에 얹을 수 있고(각 도구가 코어 커맨드 한 개 또는 체인에 1:1 매핑되므로 새 커맨드를 추가하면 새 MCP 도구를 거의 자동으로 얻는다), 내부 도구·테스트·향후 B2B 연동에 쓸 수 있다. 다만 **현 단계에서 사용자 인터페이스로 노출할 계획은 없다.** 따라서 사용자 표면 목록에는 웹앱만 둔다.

| 표면 | 대상 | 호출 경로 |
|---|---|---|
| 웹앱 프론트엔드 (TS) | 일반 시민 (유일한 사용자 경로) | → Spring REST `/api/v1/analyze` |

> **참고(비노출):** 내부 MCP 어댑터를 두는 경우, 아래 도구들이 코어 커맨드에 매핑된다. 이는 내부 구현 참고용이며 사용자 인터페이스로 제공하지 않는다.
>
> | 내부 MCP 도구(비노출) | 매핑 커맨드 |
> |---|---|
...
신규 **데이터 출처**는 커넥터 추가, 신규 **use-case**는 커맨드 추가, 신규 **클라이언트**는 표면(현재는 웹 화면; MCP 도구는 내부 전용) 추가로 각각 독립적으로 늘어난다. 세 축이 서로 결합돼 있지 않다는 점이 이 설계의 요지다.
...
런타임은 Python(추론)·Java Spring(도메인·API)·TypeScript(웹/내부 MCP) 셋이다. 이는 배포 파이프라인 3개와 서비스 간 네트워크 홉을 의미하므로, Python↔Spring 간 계약(REST 스키마)과 관측성(분산 추적)을 초기에 못박아 두는 것이 좋다.
...
1. 수직 슬라이스 1개를 끝까지 동작: 열린국회정보에서 법안 1건 수집 → 정규화 → `ImpactSummaryCommand` → **웹 화면 표시**(REST 경유).
2. 공공데이터포털·열린국회정보 API 키 발급 및 레이트리밋 정책 확정.
3. 페르소나 모델 1차 정의(일반 시민 5~6개 유형)와 `PersonaImpactCommand` 프롬프트 설계.
4. Python↔Spring REST 계약(OpenAPI 스키마) 확정 및 인용 검증기(claim ↔ 조문 ID) 규칙 확정.
5. (내부 한정) MCP 어댑터는 필요 시 내부 도구로만 추가하고, 사용자 노출 여부는 별도 의사결정으로 미룬다.
```